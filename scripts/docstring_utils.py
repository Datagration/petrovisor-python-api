...
# Required packages for the conversion pipeline
try:
    # from numpydoc.docscrape import NumpyDocString, ClassDoc, FunctionDoc
    # from sphinx.ext.napoleon import Config, NumpyDocstring, GoogleDocstring
    from sphinx.ext.napoleon import Config, NumpyDocstring
except ImportError as e:
    print(f"Error: Required package is not installed: {e}")
    print("Please install required packages with:")
    print("pip install sphinx")
    sys.exit(1)

def docstring_to_markdown(obj_name: str, obj: Any, module_path: str, config: dict=None) -> str:
    """
    ...
    if obj_type == "class":
        ...
        for cls in obj.__mro__[1:]:  # Skip first (self) in MRO
            ...
            if cls_doc == docstring:
                print(f"  Found inherited docstring from: {cls.__module__}.{cls.__name__}")
                ...
                if cls_is_external:
                    ...
                    docstring = f"A {obj_type} inheriting from {cls.__module__}.{cls.__name__}."
    ...
    try:
        ...
        first_paragraph = first_paragraph_match.group(0) if first_paragraph_match else ""
        ...
        if inspect.isclass(obj):
            ...
            def is_attribute_from_main_package(attr_name, attr_value):
                ...
                if attr_name in {'from_bytes', 'to_bytes'} and type(attr_value).__name__ == 'builtin_function_or_method':
                    return False
                ...
                if isinstance(attr_value, property) and attr_value.fget:
                    ...
                    return prop_module is None or prop_module == '' or prop_module.startswith(main_package_name)
                ...
                if hasattr(attr_value, '__module__'):
                    ...
                    return attr_module is None or attr_module == '' or attr_module.startswith(main_package_name)
                ...
                for cls in obj.__mro__:
                    if attr_name in cls.__dict__:
                        cls_module = getattr(cls, '__module__', '')
                        # Handle None module names safely
                        return cls_module is None or cls_module == '' or cls_module.startswith(main_package_name)
            ...
            for attr_name, attr_value in obj.__dict__.items():
                ...
                if hasattr(attr_value, '__class__') and attr_value.__class__.__name__ == 'Field':
                    class_attributes.append((attr_name, getattr(attr_value, 'annotation', 'Any'), attr_value))
            
            # Try to find other class variables and properties
            for attr_name in dir(obj):
                # Skip private attributes and already documented ones
                if attr_name.startswith('_') or any(a[0] == attr_name for a in class_attributes):
                    continue
                ...
                if attr_name in {'__module__', '__doc__', '__dict__', '__weakref__', '__annotations__'}:
                    continue
                ...
                if is_attribute_from_main_package(attr_name, attr_value):
                    ...
                    if is_property:
                        ...
                        if hasattr(attr_value.fget, '__annotations__') and 'return' in attr_value.fget.__annotations__:
                        ...
                        class_attributes.append((attr_name, type(attr_value).__name__, attr_value))
            ...
            if include_inherited:
                ...
                for cls in obj.__mro__:
                    ...
                    if not cls_module.startswith(main_package_name):
                        ...
                        print(f"  Skipping methods from external class: {cls.__module__}.{cls.__name__}")
                        continue
            ...
            for item in all_methods:
                ...
                if method_doc:
                    try:
                        ...
                        method_first_para = re.match(r'^(.*?)(?=\n\n|$)', method_md.strip(), re.DOTALL)
                        method_first_para = method_first_para.group(0) if method_first_para else ""
...
def simple_rst_to_markdown(rst_text):
    ...
    md_text = re.sub(
        ...
        lambda m: format_section_block("See Also", format_references_content(m.group(1).strip()), collapsible=False),
        md_text,
        flags=re.DOTALL
    )
    ...
    md_text = re.sub(
        ...
        lambda m: format_section_block("References", format_references_content(m.group(1).strip()), collapsible=False),
        md_text,
        flags=re.DOTALL
    )
    ...
    md_text = re.sub(
        r'.. warning::\s*(.*?)(?=\n\n(?:\.\.|[^\s])|$)',
        lambda m: format_section_block("Warning", m.group(1).strip(), collapsible=False),
        md_text,
        flags=re.DOTALL
    )
    ...
    md_text = re.sub(
        r'.. admonition:: (.*?)\n\n(.*?)(?=\n\n(?:\.\.|[^\s])|$)',
        lambda m: format_section_block(m.group(1), m.group(2).strip(), collapsible=False),
        md_text,
        flags=re.DOTALL
    )
...
def escape_for_mdx(text):
    ...
    text = re.sub(r'(default|optional)([^,\.]*):([^,\.]*)([,\.])', r'\1\2&#58;\3\4', text, flags=re.IGNORECASE)
...
def format_parameters_section(params_content):
    ...
    """
    param_blocks = re.findall(r':param (\w+): (.*?)(?=:param|:type|:returns|:rtype|$)', params_content, re.DOTALL)
    if not param_blocks:
        return ""
    ...
    for name, desc in param_blocks:
        # Find matching type
        type_match = re.search(fr':type {name}: (.*?)(?=:param|:type|:returns|:rtype|$)', params_content, re.DOTALL)
        ...
        default_match = re.search(r'[Dd]efault(?:s)?(?: is| are| value is)? *[=:]? *(.+?)\.?$', desc)
        if default_match:
...
def format_class_attributes(class_attributes):
    ...
    for attr_name, attr_type, attr_value in sorted(class_attributes, key=lambda x: x[0]):
...
def format_returns_section(returns_content):
    ...
    returns_match = re.search(r':returns?: (.*?)(?=:rtype:|$)', returns_content, re.DOTALL)
...
def format_interactive_examples(content):
    ...
    while i < len(lines):
        ...
        if re.match(r'^\s*>>>.*', line) and not in_code_block:
            ...
            output_lines = []
            while i < len(lines) and lines[i].strip() and not re.match(r'^\s*>>>.*', lines[i]) and not re.match(r'^\s*\.\.\..*', lines[i]):
...
def create_container_block(summary_text, content, is_open=True, icon=None):
    ...
    if "detailed documentation" in summary_text.lower():
        ...
        elif any(word in title_lower for word in ["info", "note", "see also", "references"]):
            admonition_type = "info"

    return create_admonition(admonition_type, content, title=title, is_open=is_open, collapsible=True)
